object Exercises {
    ...
    def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, func: (Vector2D, Vector2D) => Double, rightVec0:
    Vector2D, rightVec1: Vector2D) = func(leftVec0, leftVec1) + func(rightVec0, rightVec1)
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray
          .mapValues(radiusAndDensity => calculateBallWeight(radiusAndDensity._1, radiusAndDensity._2))
          .toSeq
          .sortBy(ball => ball._2)
          .map(ball => ball._1)
    }
